 ...
 * 
 * the lock object syntax is a shorthand notation for Monitor.Enter() and Monitor.Exit()
 * */
        ...
        public void CountUp(Action<long> callbackFunction)
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= Count; i++) // we use the property here, in the same class to remove the need for parameterizing the thread
                {
                    ...
                    // announce this thread is waiting for the lock until it is released for the first time (acquire the lock)

                    lock (Shared.SharedLock) // we enclose the critical code inside the lock statement within curly braces and it is compiled as Monitor.Enter() and Monitor.Exit()
                    {
