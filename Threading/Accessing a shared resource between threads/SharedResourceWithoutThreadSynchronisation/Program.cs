/* When multiple threads access the same variable, property or resource , this is called as a shared resource.
 * Race condition appears between threads, when due to uncertainty of the order in which the threads are executed, threads attempt and modify the shared resource in an inconsistent manner.
 * Deadlock occurs when two threads wait for a shared resource to be released by the other thread, and program unresponsivess occurs
 * Starvation occurs when a thread with a higher priority had priviledged access over a resource, and the second thread is unable to use therefore.
 * */
    ...
    public class NumbersCountUp
    {
        ...
        public void CountUp(Action<long> callbackFunction)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("the shared resource is " + Shared.SharedResource);
                    Shared.SharedResource++;
                    Console.Write("the shared resource after ++ is " + Shared.SharedResource);
