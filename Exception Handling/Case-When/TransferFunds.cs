    ...
    internal class TransferFunds
    {
        public void TransferFund(Account sourceAccount, Account destinationAccount, double amount)
        {
            try
            {
                if(amount > 5000)
                {
                    throw new ArgumentOutOfRangeException("source", 5000, "This is called from the inner method: argument is out of range");
                }

                if(amount > sourceAccount.CurrentValue)
                {
                    throw new InsufficientFundsException($"This is called from the inner method. The account value is {sourceAccount.CurrentValue} and you wish to transfer {amount} "); //this creates an object of the class InsufficientFundsException() /Custom Exception  which will be called in the main() method
                }
